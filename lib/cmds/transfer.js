...
function getFullTransferObjects (iota, transfers, opts = { removeChecksum: false }) {
  //start [custom IOTAcooler] tag
  //const meta = { message: '', tag: '9'.repeat(27) }
    assert.ok(smidgen.config.tag.length == 27, 'tag must be 27 char long')
    const meta = { message: '', tag: smidgen.config.tag } //use custom tag [IOTACOOLER edit]
  //end
